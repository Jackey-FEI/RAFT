	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				if ok {
					index = index1
					log.Printf("test check index %v, on leader: %v with command: %v\n", index1, si, cmd)
					break
				}
			}
		}
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				log.Printf("test find: %v, on command: %v, expected command: %v on servernum %v\n", nd, cmd1, cmd1.(int), expectedServers)
